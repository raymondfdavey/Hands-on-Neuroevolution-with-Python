...
class ParallelEvaluator(object):
    def __init__(self, trialSim, num_workers, eval_function, timeout=None, maxtasksperchild=None):
        """
    ...
    def evaluate(self, genomes, config):
        ...
        for ignored_genome_id, genome in genomes:
            jobs.append(self.pool.apply_async(self.eval_function, (self.trialSim, genome, config)))
...
def eval_fitness(sim, genome, config, time_steps=400):
    """
    ...
    maze_env = copy.deepcopy(sim.orig_maze_environment)
...
#                  hyper-parameters
#     """
#     num_workers = os.cpu_count()  # get number of available CPU cores
#     multiprocessing.set_start_method('spawn')  # Use spawn to start new processes
#     evaluator = ParallelEvaluator(eval_function=eval_fitness, timeout=None, num_workers=num_workers)
...
def run_experiment(config_file, maze_env, trial_out_dir, args=None, n_generations=100, silent=False):
    ...
    pe = neat.ParallelEvaluator(trialSim, multiprocessing.cpu_count(), eval_fitness)
    best_genome = p.run(pe.evaluate, n=n_generations)
